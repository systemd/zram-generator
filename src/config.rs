...
impl Device {
    ...
    fn set_disksize_if_enabled(&mut self, memtotal_mb: u64) -> Result<()> {
        ...
        if self.zram_fraction.is_some() || self.max_zram_size_mb.is_some() {
            // deprecated path
            self.disksize =
                (self.zram_fraction.unwrap_or(0.5) * memtotal_mb as f64) as u64 * 1024 * 1024;

            if let Some(max_mb) = self.max_zram_size_mb.unwrap_or(None) {
...
fn parse_line(dev: &mut Device, key: &str, value: &str) -> Result<()> {
    match key {
        ...
        "zram-fraction" => {
            warn!(
                "{}: deprecated {} configuration key set. Use zram-size = instead!",
                dev.name, key
            );
