...
fn write_contents(output_directory: &Path, filename: &str, contents: &str) -> Result<()> {
    static CURRENT_EXE: Lazy<&[u8]> = Lazy::new(|| {
        env::current_exe()
            .map(|ce| Box::leak(ce.into_boxed_path()).as_os_str().as_bytes())
            .unwrap_or_else(|_| concat!(crate_name!(), " ", crate_version!()).as_bytes())
    });

    let path = output_directory.join(filename);
    make_parent(&path)?;

    // TODO: ideally this'd be write_all_vectored and we'd both Not Allocate
    //       /and/ complete in one syscall, but that's an unstable feature
    fs::File::create(&path)
        .map(io::BufWriter::new)
        .and_then(|mut f| {
            f.write_all(b"# Automatically generated by ")?;
            f.write_all(*CURRENT_EXE)?;
            f.write_all(b"\n\n")?;
