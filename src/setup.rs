...
pub fn run_device_setup(devices: Vec<Device>, device_name: String) -> Result<()> {
    let device = devices.into_iter().find(|dev| dev.name == device_name).ok_or_else(|| anyhow!("Device {} not found", device_name))?;
