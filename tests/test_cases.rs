...
fn test_generation(path: &str) -> Result<Vec<config::Device>> {
    ...
    // Files with backslashes are generated, but have been removed from the expected
    // output because 'cargo publish' refuses to package such file names.
    let diff = Command::new("diff")
        .arg("-u")
        .arg("--recursive")
        .arg("--exclude=.empty")
        .arg("--exclude=\\\\x2e\\\\xc5\\\\xbcupan\\\\x2dci3p\\\\xc5\\\\x82y.mount")
